...
def welcome():
    ...
    if request.method == "POST":
        ...
        if typ_forma == 'saldo':
            ...
            if saldo is None:
                first_saldo = Saldo(saldo=newsaldo)
                db.session.add(first_saldo)
                db.session.commit()
                saldo = first_saldo
    ...
    return render_template('index.html', last_saldo=last_saldo, przedmioty=przedmioty)
